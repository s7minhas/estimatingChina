...
%DIF DEL china_3who_initSubmit.tex   Sat Jan 23 11:49:35 2021
%DIF ADD china_3who_reSubmit.tex     Sat Jan 23 11:49:35 2021
...
\DIFdelbeginFL %DIFDELCMD < \subfloat[Foci network\label{fig:focinet}]{\noindent \begin{centering}
%DIFDELCMD < \includegraphics[width=2.5in]{figure/data_fig1}
%DIFDELCMD < \par\end{centering}
%DIFDELCMD < }\subfloat[Appearance network\label{fig:appnet}]{\noindent \begin{centering}
%DIFDELCMD < \includegraphics[width=2.5in]{figure/data_fig2}
%DIFDELCMD < \par\end{centering}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Foci network\label{fig:focinet}]{\noindent \begin{centering}
...
\begin{figure}[H]
\noindent \begin{centering}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\textwidth]{figure/freq}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\textwidth]{freq}
\DIFaddendFL \par\end{centering}
...
\begin{figure}[H]
\noindent \begin{centering}
\DIFdelbeginFL %DIFDELCMD < \subfloat[Xi and Li\label{fig:Xi_Li}]{\noindent \begin{centering}
%DIFDELCMD < \includegraphics[width=.8\textwidth]{figure/Xi_Li}
%DIFDELCMD < \par\end{centering}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Xi and Li\label{fig:Xi_Li}]{\noindent \begin{centering}
\includegraphics[width=.8\textwidth]{Xi_Li}
\par\end{centering}
}
\DIFaddendFL \par\end{centering}
\noindent \begin{centering}
\DIFdelbeginFL %DIFDELCMD < \subfloat[Xi and Zhang\label{fig:Xi_ZG}]{\noindent \begin{centering}
%DIFDELCMD < \includegraphics[width=.8\textwidth]{figure/Xi_ZG}
%DIFDELCMD < \par\end{centering}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Xi and Zhang\label{fig:Xi_ZG}]{\noindent \begin{centering}
...
\begin{figure}[ht]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.9\textwidth]{figure/dvViz_names2}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.9\textwidth]{dvViz_names2}
\DIFaddendFL \caption{The complete coappearance network. Names of elites depicted in the network are scaled by their number of appearances and edges between actors indicate that they appeared together at an event.}
...
To answer the question of ``who are my friends,'' we utilize a latent factor model (LFM) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Hoff2002,Hoff2005,Minhas2016a}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Hoff2005,Minhas2016a}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The LFM positions actors in a $k$ dimensional latent vector space based on third order dependence patterns. In this space, actors whose vectors point in similar dimensions are more likely to share similar preferences and be members of the same latent coalitions. The angles between these vectors then provides a measure of the extent to which the preferences and factional links are similar. %DIF < Given its ability to capture latent affinities between interconnected actors, LFM has been used to infer state foreign policy preferences \citep{gallop:minhas:2018}.
\DIFaddbegin \DIFadd{Given its ability to capture latent affinities between interconnected actors, LFM has been used to infer state foreign policy preferences \mbox{%DIFAUXCMD
...
For our interest in latent affinity, the key output is $U\Lambda U^{\top}$, where $U$ is an $n\times k$ matrix that represents each actors vector in the $k$-dimensional latent network.\DIFaddbegin \footnote{\DIFadd{We estimate a 2-dimensional latent factor space for our application.}} \DIFaddend It captures the effect of stochastic equivalence and homophily on official appearances. But a caveat in interpreting this space is that it is non-Euclidean, as the actor vectors are embedded within a $k$-dimensional hyper sphere. We thus cannot simply use distances in this space as a measure of latent affinities. Rather, the important measure here is an actor's vector in this space, and the similarity between the vector of one actor and another.

\noindent \begin{center}
\begin{figure}[H]
\noindent \begin{centering}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in]{figure/uViz}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=5in]{uViz}
\DIFaddendFL \par\end{centering}
...
\begin{figure}[H]
\noindent \begin{centering}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=3.5in]{figure/LSG_full}
%DIFDELCMD < %%%
...
\begin{figure}[H]
\noindent \begin{centering}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\textwidth]{figure/effects}
%DIFDELCMD < %%%
