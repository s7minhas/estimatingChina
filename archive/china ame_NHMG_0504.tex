...
A Latent Space Approach to Understanding Elite Coappearances in China}\thanks{The replication files will be made available online.}}
\maketitle
\begin{abstract}
\begin{flushleft}
...
using a latent factor network model and a dataset that tracks about
10000 appearance records of over 200 top CCP elites from 2013 to 2017.
We test how well these three indicators fare in predicting elites’
...
analysis of texts, therefore, allow us to reveal ups and downs of
key elites in a dynamic way.\footnote{For similar studies, see the review of \citet{Ban2018}.}
...
disaggregate the strategic factors into three questions — i.e., who's
in charge, who do I work with, and who are my friends, which correspond
to the individual level characteristics, dyadic links, and latent
affinities. Generally speaking, our approach can be summarized as
follows: after controlling for random noise, powerful elites (i.e.,
...
cosine of the angle formed by the latent vectors of both actors.\footnote{This can be calulated in the following way, where $u$ represents
...
has emerged. Yet, two surprising overlapping (Yu Zhengsheng and Liu
Yunshan) and (Wang Qishan and Zhang Dejiang) {]}

{[}An interesting pattern, while Xi was distant from Li, the premier
of Li Keqiang, all four vice premiers (i.e., Zhang Gaoli, Liu Yandong,
Wang Yang, and Ma Kai). While this peculiarity cannot be explained
by institutional/legal explanations, which would suggest vice premiers
cooperate more the premiers, it is consistent with studies like Jaros
and Pan{]}

\section{Appointments in Leading Small Groups}

To test the validity of our measures, we attempt to estimate membership
on the Leading Small Groups (LSG) using measures of latent similarity
we discuss above. LSGs, though not incorporated into charts of Party
or government organs, play a pivotal role in communicating, coordinating,
and implementing of important decisions across different segments
and various levels of the governments. LSGs, therefore, not only can
serve as an institutional remedy to the regime's prolonged problem
of political fragmentation, but also, as shown by the development
of the infamous Central Cultural Revolution Group (1966-69), LSGs
could become an effective vehicle for expansion of personal power.

In this study we examine LSGs at the national level as well as their
members, and we further differentiate between Central Committee (\emph{zhongyang
lingdao xiaozu}, Party LSGs) and State Council Leading Small Groups
(\emph{guojia}, \emph{quanguo}, \emph{guowuyan}, or \emph{zhongguo
lingdao xiaozu}, State LSGs). All LSGs are involved in the collecting
and providing policy intelligence, and coordinate among different
stakeholder interests. However, only a small share of them have coordination
(\emph{xietiaozu}) in their name. The ones that do, often include
stakeholders that are strictly speaking outside of the party-state
bureaucracy, such as business associations.
\noindent \begin{center}
\begin{figure}[H]
\noindent \begin{centering}
...
unique entries of affiliation relationship.
...
\begin{figure}[H]
\noindent \begin{centering}
\includegraphics[width=4in]{figure/betaEst}
\par\end{centering}
\caption{Estimates of Negative Binomial Analysis of Number of Total LSGs}
\end{figure}
\par\end{center}

\noindent \begin{center}
\textbf{{[}Figure 6 is about here.{]}}
\par\end{center}

\noindent \begin{center}
\begin{figure}[H]
\noindent \begin{centering}
\includegraphics[width=5in]{figure/effects}
\par\end{centering}
\caption{Estimated Effects of Latent Distance to Xi}
\end{figure}
\par\end{center}

\noindent \begin{center}
\textbf{{[}Figure 7 is about here.{]}}
\par\end{center}
...
\small
  ...
  \hline \\[-1.8ex]
  & & & & Dawid & \\
  Model & Logarithmic & Brier & Spherical & Sebastiani & RMSE\\    \hline \\[-1.8ex]
  Total appearance model (1) & 1.77 & -0.36 & -0.64 & 2.70 & 1.89 \\
  Total appearance and coappearance model (2) & 1.77 & -0.38 & -0.66 & 2.65 & 1.84 \\
  Total appearance and latent distance model (3) & 1.67 & -0.51 & -1.04 & 2.41 & 1.72  \\ \\[-1.8ex]
    \hline
 \hline \\[-1.8ex]
\end{tabular} 
\end{table}
...
\noindent \begin{center}
\textbf{{[}Figure 8 is about here.{]}}
\par\end{center}

\noindent \begin{center}
\begin{figure}[H]
\noindent \begin{centering}
\includegraphics[width=4in]{figure/XLcoefs}
\par\end{centering}
\caption{Estimates of Negative Binomial Analysis: Comparing distances to Xi
and Li}
\end{figure}
\par\end{center}

\noindent \begin{center}
\textbf{{[}Table 3 is about here.{]}}
\par\end{center}

\noindent \begin{center}
\begin{table}[H]
\caption{Out-of-sample Performance on Scoring Rule Metrics: Comparing Xi and
Li}
...
\centering
  ...
  \hline \\[-1.8ex]
   ...
   Total appearance with latent distance, Xi (3) & 1.67 & -0.51 & -1.04 & 2.41 & 1.72 \\
   Total appearance with latent distance, Li (8) & 1.73 & -0.52 & -1.09 & 2.58 & 1.86 \\
   Total appearance and latent distances, Xi and Li (9) & 1.74 & -0.51 & -1.07 & 2.62 & 1.85 \\ \\[-1.8ex]     \hline 
 \hline \\[-1.8ex]
\end{tabular} 
\end{table}
\par\end{center}

\section{Differentiating between Central and State LSGs}

{[}max and i thought that the separate analysis by lsg should come
after the additional analysis with li{]}
...
or State LSGs. Again in this analysis, latent distance from Xi has
a consistent negative effect for both types of LSGs.
\noindent \begin{center}
\textbf{{[}Table 2 is about here.{]}}
\par\end{center}
...
\small
\centering
  \begin{tabular}{@{\extracolsep{0pt}} l D{.}{.}{2.3} D{.}{.}{2.3} D{.}{.}{2.3} D{.}{.}{2.3} D{.}{.}{2.3} D{.}{.}{2.3} D{.}{.}{2.3} D{.}{.}{2.3}}
 \\[-1.8ex]\hline
 \hline \\[-1.8ex]
 \\[-1.8ex] & \multicolumn{2}{c}{Model 4} & \multicolumn{2}{c}{Model 5} & \multicolumn{2}{c}{Model 6} & \multicolumn{2}{c}{Model 7}\\
 \\[-1.8ex] & \multicolumn{1}{c}{CC} & \multicolumn{1}{c}{SG} & \multicolumn{1}{c}{CC} & \multicolumn{1}{c}{SG} & \multicolumn{1}{c}{CC} & \multicolumn{1}{c}{SG} & \multicolumn{1}{c}{CC} & \multicolumn{1}{c}{SG} \\
 \hline
  & & & & & & & & \\
 Total appearances & 0.062^{***} & 0.002  &   &  &  &                                             & 0.055^{***} & -0.0004 \\
         &(0.017)        &(0.003) &   &  &  &                                             &(0.017)       & (0.003) \\
 Coappearances with Xi  &               &        & 0.219^{***} & 0.016 &  &                            &       &     \\
         &               &        &(0.072)        &(0.016)&  &                            &       &     \\
 Latent distance to Xi &               &        &               &       &-1.134^{***} & -0.708^{***} & -0.541^{*}  & -0.727^{***}   \\
               &               &        &               &       &(0.278)        &(0.255)        &(0.302)       & (0.274)  \\
 & & & & & & & & \\
 Intercept 1 & \multicolumn{2}{c}{$-1.082^{***}$} & \multicolumn{2}{c}{$-0.794^{***}$} & \multicolumn{2}{c}{$0.474^{*}$} & \multicolumn{2}{c}{$-0.534$} \\
             & \multicolumn{2}{c}{(0.175)}        & \multicolumn{2}{c}{(0.145)}        & \multicolumn{2}{c}{(0.264)}     & \multicolumn{2}{c}{(0.354)}        \\
 Intercept 2 & \multicolumn{2}{c}{$-0.646^{***}$} & \multicolumn{2}{c}{$-0.657^{***}$} & \multicolumn{2}{c}{$0.023$}     & \multicolumn{2}{c}{0.046} \\
             & \multicolumn{2}{c}{(0.137)}        & \multicolumn{2}{c}{(0.134)}        & \multicolumn{2}{c}{(0.256)}     & \multicolumn{2}{c}{(0.294)}        \\
 Intercept 3 & \multicolumn{2}{c}{$0.913^{**}$}   & \multicolumn{2}{c}{$0.920^{**}$}   & \multicolumn{2}{c}{$0.648^{*}$} & \multicolumn{2}{c}{$0.826^{**}$} \\
             & \multicolumn{2}{c}{(0.398)}        & \multicolumn{2}{c}{(0.379)}        & \multicolumn{2}{c}{(0.365)}     & \multicolumn{2}{c}{(0.407)}        \\
  & & & & & & & & \\
 \hline
 \hline \\[-1.8ex]
...
— 1) who's in charge, (2) who do I work with, and (3) who are my friends
— by examining elites’ total appearances, dyadic coappearances, and
finally their latent network distance. We find that latent proximity
